...
estimate_phase_scan_by_hilbert_transform(phase_scan_record_view<Float> const &record)
{
  auto const centered_ground = hilbert::analysis::remove_dc_component(record.ground());
  auto const centered_force = hilbert::analysis::remove_dc_component(record.force());
  ...
  auto const ground_window = record.measurement_range().slice(std::span<std::complex<Float> const>{analytic_ground});
  auto const force_window = record.measurement_range().slice(std::span<std::complex<Float> const>{analytic_force});
