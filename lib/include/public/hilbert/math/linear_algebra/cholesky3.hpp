...
class cholesky_factor3
{
  ...
  cholesky_decompose(symmetric_matrix<OtherFloat, 3uz> const &matrix);

public:
  [[nodiscard]]
  vector<Float, 3uz>
  solve(vector<Float, 3uz> const &right_hand_side) const
  {
    ...
    vector<Float, 3uz> const result{x0, x1, x2};
...
cholesky_decompose(symmetric_matrix<Float, 3uz> const &matrix)
{
